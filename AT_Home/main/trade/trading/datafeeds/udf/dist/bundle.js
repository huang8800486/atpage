! function(e, t) {
	...
	var i = function() {
			...
			return e.prototype.getBars = function(e, t, r, s) {
				...
				localStorage.interv = t;
				return new Promise(
					function(a, u) {
						/** *********custom begin************* */
						if(sessionId){
							i['sessionId'] = sessionId;
							o._requester.sendRequest(o._datafeedUrl, "api/history", i).then(function(e) {
							...
							var url = wssPoint + "/api/market/kline";
							var webSocket = new WebSocket(url);
							webSocket.onerror = function(event) {
								console.log("webSocket error:"+event.data);
							};
							webSocket.onopen = function(event) {
								console.log("kline webSocket connect..");
							};
							webSocket.onmessage = function(event) {
								var J = JSON.parse(event.data);
								if(J.status == "connect_success"){
									// get session id. send to server
									if(J.data){
										sessionId = J.data;
										i['sessionId'] = J.data;
									}
								...
								}else if(J.status == "query_success"){
									// push data
									var socketResult = J.data;
									var Kline = socketResult.chart;
									if(Kline && Kline.s != 'no_data'){
										for(var s = void 0 !== Kline.v, oo = void 0 !== Kline.o, ii = 0; ii < Kline.t.length; ++ii) {
											var n = {
												time: 1e3 * Kline.t[ii],
												close: Number(Kline.c[ii]),
												open: Number(Kline.c[ii]),
												high: Number(Kline.c[ii]),
												low: Number(Kline.c[ii])
											};
											oo && (n.open = Number(Kline.o[ii]), n.high = Number(Kline.h[ii]), 
													n.low = Number(Kline.l[ii])), s && (n.volume = Number(Kline.v[ii])), newDataCallback(n);
										}
									}
								}
							};
...
//					resolution: o
//				};
//				this._send("api/marks", i).then(function(e) {
//					if(!Array.isArray(e)) {
...
//				})
			...
			this._configuration.supports_time && this._send("api/time").then(function(e) {
				var t = parseInt(e);
				isNaN(t) || r(t)
			}).catch(function(e) {
			...
			if(this._configuration.supports_group_request) {
				...
				this._send("api/symbols", i).then(function(e) {
			...
			return this._send("api/config").catch(function(e) {
