...
Kline.prototype = {
    ...
    websocketRedister: function(symbol) {
        var $this = this;
        /*$.get('http://127.0.0.1/TP5.1/kapi/public/depth?depth=' + symbol + '&v=' + Math.random(), function(result) {
        ...
        /*socket.on('messge' + $this.symbol, function(result) {
            console.log(result);
            result = JSON.parse(result.depth);
            console.log(result);
            $this.updateDepth(result);
        });
        return false;*/
        ...
        ;
        var target = "wss://trades.btc123.com/kline";
        ...
        this.klineWebsocket.onopen = function() {
            $this.klineWebsocket.send("{'symbol':'" + symbol + "'}")
        }
        ;
